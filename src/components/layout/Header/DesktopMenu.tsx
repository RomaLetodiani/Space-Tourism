...
const DesktopMenu = () => {
  ...
  return (
    <nav className="bg-[#ffffff0a] relative backdrop-blur-2xl text-white px-10 lg:pl-28 lg:pr-36">
      {isAboveDesktop && (
        <div
          className={`absolute w-1/2 border opacity-25 translate-y-1/2 translate-x-[10%] top-1/2 -left-1/2 z-30 bg-white transition-transform duration-300 ease-in-out`}
        ></div>
      )}
          ...
          <Link key={item.index} to={item.link}>
            <li
              className={`cursor-pointer tracking-[2.7px] text-sm py-8 ${
                location.pathname === item.link ? 'border-b-4' : ''
              } border-t-4 border-t-transparent font-barlowCondensed`}
            >
              {isAboveTablet && (
